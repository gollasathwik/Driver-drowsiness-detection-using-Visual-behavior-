                    ...
                    if mar > MOUTH_AR_THRESH:
                            ...
                            if MOUTH_COUNTER >= MOUTH_AR_CONSECUTIVE_FRAMES:
                                    ...
                                    if YAWN_COUNT > 5:
                                        ...
                                        engine.runAndWait()
                                        #audioalert()
                                        bot = telegram.Bot(token='6061089686:AAEixUS6bmUxteqkHEg9IPKY3a68Zh0bZGc')
                                        chat_id = '857465702'
                                        async def send_telegram_alert():
                                              ...
                                              message_text += f"\n\nDriver's current location: {location_url}"
...
# Send the message
                                async def send_telegram_alert(message_text):
                                      await bot.send_message(chat_id=chat_id, text=message_text)
                                asyncio.run(send_telegram_alert("Driver is feeling drowsy. Please take necessary actions."))
                    ...
                    if pitch >150 and pitch<210:
                        ...
                        cv2.putText(frame, "looking to the right".format(ear), (30, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        engine.say("lookingto the right!")
                        engine.runAndWait()
                    elif pitch >270 and pitch <290:

                        cv2.putText(frame, "looking to the left".format(ear), (30, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        engine.say("looking to the left!")
                        engine.runAndWait()
